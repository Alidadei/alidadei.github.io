...
“下面的这几章介绍，放到真实的工作场景来讲，不要写一堆抽象的名词。比如：什么是stdout 和transcript？？直接用真实的AIcoding场景来讲！”
...
“我需要的是auto research场景，比如：在晚上自动跑一整套参数优化并记录（实验运行的时候不需要agent，运行完毕后，总结，然后继续跑新的实验……）、自动完成配置环境+复现的全部工作”
