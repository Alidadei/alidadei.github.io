...
大语言模型首先会在海量文本上做**预训练**来吸收必要的知识，其本质是让模型通过 **自监督地预测下一个 token** 这一过程 学会文本中的各种规律并将其压缩进模型参数里——所谓“自监督”，就是正确答案直接来自原文本的下一个 token，不需要人工逐条标注；之后，模型还需要学会贴近人的偏好、遵守某些特定的输出格式或者在特定任务上提高正确率，针对这些能力的训练可以统称为 **后训练（post-training）**。
...
为什么这能表示“信息上的背离”？在信息论里，一个结果发生时携带的信息量可以写成 $-\log p(x)$：原本越不可能发生的结果，一旦真的发生，就越让人意外，信息量也越大。于是，对同一个结果 $x$：
...
**所以 KL divergence 的大白话含义就是：真实结果来自 $P$，却用 $Q$ 去理解或预测这些结果时，平均要多付多少“信息账单”。** 使用自然对数时单位叫 **nat**，使用以 2 为底的对数时单位叫 **bit**。
...
这里 B 对应的第二项其实是负数，因为 $Q$ 给 B 的概率比 $P$ 更高；这不矛盾，KL 保证的是所有项加起来不小于 0，并不是每一项都不小于 0。最终的 0.368 也不是说“两个分布相差 36.8%”，而是：如果结果真的按照 $P$ 产生，使用 $Q$ 而不是 $P$ 来描述结果，平均每次会多付约 0.368 nat 的信息代价。这个“平均多付的信息量”越接近 0，$Q$ 就越贴近 $P$。[Stanford EE376A 信息论讲义](https://web.stanford.edu/class/ee376a/files/lecture_notes.pdf)也将 KL 写成 $\mathbb E_P[\log(P/Q)]$，并称为 relative entropy（相对熵）。
...
- **2 个数据并行副本**：两张卡各放一份参数相同的 LLM，同时处理各自拿到的数据，最后合并梯度。它们通常拿到的是不同训练样本，并不等于“把同一道题原封不动算两遍”。GRPO 可能把同一 prompt 的多条不同 completion 分到不同设备，但这是 sampler 怎样分组的结果，不是数据并行强制要求的。[PyTorch DDP 文档](https://docs.pytorch.org/docs/stable/generated/torch.nn.parallel.DistributedDataParallel.html)也把它定义为多个模型副本之间同步梯度，输入怎样切分则由数据加载器负责。
...
这里一条 **completion** 就是前文说的一条完整回答，不是一道问题。64 条 completion 表示 64 条回答；如果 GRPO 对每条 prompt 正好生成 $G=8$ 条回答，而且这 64 条回答由 8 个完整分组组成，那么它们对应 8 条 prompt：
