...
这一部分涉及的概率论对于理解后训练的内容尤为重要。

LLM在吐出下一个 token 的时候其实做了一个基于概率的转换过程：首先，为词表中每个 token 给出一个原始打分，叫做 **logit**；然后把这个分数转换成概率，最终再根据概率进行解码（选择 token）输出。
...
**一条 LLM 已经输出的回答对应的 $-\log p_\theta(y\mid x)$，准确来说叫 surprisal（自信息，也常译为惊奇度），不是 entropy 本身。** Surprisal 描述的是“这个已经发生的结果有多出乎意料”，entropy 则是所有可能结果的 surprisal 按照各自概率取平均。[Stanford EE376A 信息论讲义](https://web.stanford.edu/class/ee376a/files/lecture_notes.pdf)把完整回答分布的 entropy 写成：
...
所以，如果回答 $y$ 确实是从模型分布中采样出来的，$-\log p_\theta(y\mid x)$ 可以作为估计完整回答分布 entropy 的一个蒙特卡洛样本；但是对于这一条已经抽到的回答，这个数本身仍然叫 surprisal。直接把它叫做“这条回答的 entropy”并不准确。

工程日志里的 `entropy` 还要看具体实现，不能只看指标名猜。当前 [TRL GRPOTrainer](https://huggingface.co/docs/trl/main/en/grpo_trainer) 记录的是“生成回答中各 token 预测分布的平均 entropy”：沿着回答，在每个生成位置计算一次整个词表概率分布的 entropy，再对有效 token 求平均。若只看一条长度为 $T$ 的回答，可以写成：
...
这个指标既不是这条回答的总 surprisal，也不是把所有可能回答完整枚举后精确算出的序列 entropy。
...
一个分布当然可以有不止一座峰。这里的横轴不是“一条回答从开头写到结尾的位置”，而是把同一个 prompt 的许多可能回答按照“简洁 → 详细”粗略排成一条轴；纵轴表示模型分给这一类回答的概率。如果简洁回答和详细回答都很常见，夹在中间的回答反而较少，这个分布就会有两个高点，也就是两个 mode。

如果 $Q$ 有足够的表达能力，并且真的优化到全局最优，那么无论用哪个方向的 KL，最优结果都是 $Q=P$。下面的差异主要出现在 $Q$ 能力有限、只能用更简单的形状逼近 $P$ 时。图中 reverse KL 选择了简洁流派 A 只是举例，它也可能选择流派 B：

```text
回答风格              简洁流派 A       两类之间       详细流派 B
目标分布 P               ██████            ▏              ██████
Q（forward KL）          █████            ███             █████
Q（reverse KL）          ██████            ·                 ·
```
...
因此，模型每次输出的 token 是 action，**Policy 是模型决定“在当前上下文里，下一个 token 各有多大概率”的整套规则**。写成 $\pi_\theta(a_t\mid s_t)$ 时，表示模型在状态 $s_t$ 下输出具体 token $a_t$ 的概率。这套对应关系也可参考[这篇关于 LLM 强化学习的说明](https://zhuanlan.zhihu.com/p/1990178700575130664)。

#### 当前策略、旧策略与参考策略

先别把“策略”想成一种脱离 LLM 的神秘东西。假设我们拿 `Qwen2.5-7B-Instruct` 的一份 SFT checkpoint 做 PPO，下面只是为了对得上号，不是在复述 Qwen 的真实训练配置：
...
- **当前策略（current policy）**：就是此刻正在被优化器更新的那份 Qwen 模型，记作 $\pi_\theta$。模型参数一更新，它在相同上下文里给出的下一 token 概率也会变化。
- **行为策略（behavior policy）**：就是实际生成这批训练回答的那份模型。在上面的 on-policy 例子里，它是 current policy 在生成时刻的版本；如果回答来自另一个模型或更早的 checkpoint，行为策略就是那个模型。论文里常记作 $\mu$ 或 $\pi_b$，但符号并不统一。
...
- **参考策略（reference policy）**：就是从训练起点冻结下来的那份 Qwen 模型，通常是 SFT checkpoint，记作 $\pi_{\mathrm{ref}}$。它不负责生成本批训练数据，而是长期提醒 current policy“不要离起点太远”。

旧策略会随着 rollout 批次更新；参考策略往往长期不变，这俩一个负责回答“这批数据是谁生成的”，另一个负责约束“模型不要离初始行为多远”。

后面还会遇到两份不叫 policy、但同样容易被写抽象的东西：

- **reward model**：另一份冻结的模型，输入 prompt 和完整回答，输出一个分数；如果使用数学判题器或单元测试做 RLVR，这里可能根本没有 LLM。
- **value model / critic**：PPO 额外训练的估分模型，输入 prompt 加当前回答前缀，输出“从这里继续生成，预计最终能拿多少分”。它可以是独立模型，也可以和 policy 共用部分骨干网络，但职责不是生成回答。GRPO 通常不需要这份独立 critic。

当前 [TRL PPOTrainer](https://huggingface.co/docs/trl/main/ppo_trainer) 的接口也直接把这些对象列成 `model`、`ref_model`、`reward_model` 和 `value_model`。换句话说，这些名词在代码里通常真的是不同的模型对象、模型副本或某个时刻保存下来的 logprob，不是四种空泛的“策略思想”。
...
继续用上面的 Qwen PPO 训练举例。假设 prompt 是一道数学题，current policy 生成完整答案后，判题程序给了 1 分；任务只在结尾打分，$\gamma=1$，并暂时忽略 GAE：
...
- **policy model / actor**：正在训练的那份 Qwen，优化器真正修改的是它；
- **old policy**：同一份 Qwen 生成本批回答时的快照或保存下来的 logprob，用于计算新旧概率比；
- **value model / critic**：输入 prompt 和回答前缀、输出一个 value 的估分模型，用于计算优势；
- **reference model**：从 SFT 起点冻结下来的那份 Qwen，用 KL 约束 current policy 不要偏离太远；
- **reward model 或 verifier**：读取完整回答并给分，通常在 policy 训练时保持不变。

所以“PPO 的 critic 负责打奖励分”是错误说法：奖励源和 critic 是两种角色。

#### GRPO

[DeepSeekMath](https://arxiv.org/abs/2402.03300) 提出的 Group Relative Policy Optimization（GRPO）保留了 PPO 式的策略更新思路，但省去独立 critic。对应到上面的 Qwen 例子：仍然有一份正在更新的 policy model，也仍然需要 reward model 或 verifier；如果启用 KL 约束，还会有冻结的 reference model。少掉的是专门预测 value 的那份模型，基线改由同一 prompt 下的一组回答互相比较得到。

具体来说，让 current policy 对同一个 prompt 采样 $G$ 个回答，得到奖励 $R_1,\ldots,R_G$，原始论文中的组内标准化优势可简化写成：
...
[Direct Preference Optimization](https://arxiv.org/abs/2305.18290) 使用固定的 chosen/rejected 数据，直接提高 chosen 相对 rejected 的偏好概率。

对应到上面的 Qwen 例子，标准 DPO 只需要两份语言模型角色：`model` 是正在训练的 Qwen，`ref_model` 是训练开始时冻结的 Qwen；chosen/rejected 来自固定数据集。训练过程中不让 current policy 现场 rollout，因此没有“生成本批回答的 old policy”；也不需要 PPO 的 value model，更不需要先单独训练一份 reward model。当前 [TRL DPOTrainer](https://huggingface.co/docs/trl/main/dpo_trainer) 的接口也直接对应 `model` 和 `ref_model`。
...
以下以原始论文、研究机构技术报告和主流框架官方文档为主，最后检索于 2026 年 7 月 18 日。链接用于追溯定义与事实，不表示不同来源的全部工程选择都完全一致；未固定版本的框架文档以后仍可能变化。
