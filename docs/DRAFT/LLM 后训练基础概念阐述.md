...
如果约定 $P$ 是目标（或叫 reference）、$Q$ 是待学习策略，那么常见称呼是：

- forward KL：$D_{\mathrm{KL}}(P\|Q)$；

- reverse KL：$D_{\mathrm{KL}}(Q\|P)$。

下面直接用一张图看两个方向究竟会选谁。$P$ 是目标分布；为了方便对比，我手工设定了“铺开”和“只押 A”两个不完美的候选 $Q$，图中数值只是教学示例，不是真实模型的统计结果。

![目标分布 P 与两个候选 Q 的概率分布示意图](../../public/images/posts/llm-kl-modes.svg)

横轴把同一个 prompt 的可能回答粗略排成“简洁正确 A—别扭混合—详细正确 B”，纵轴是每类回答的概率。$P$ 在 A 和 B 处的概率都高，这两个高概率区域就是两座 **mode（模式，也就是概率山峰）**。

- **Forward KL：$D_{\mathrm{KL}}(P\|Q)$** 由 $P$ 决定每一项说话的音量。$P$ 给 B 的权重是 0.495，而 $Q_{\text{只押 A}}$ 只给 B 0.01，因此漏掉 B 会被重罚；最终“铺开”的 KL 是 0.64，小于“只押 A”的 1.59，所以 forward KL 选择铺开，表现出 **mode-covering（覆盖模式）**的倾向。
- **Reverse KL：$D_{\mathrm{KL}}(Q\|P)$** 改由正在接受检查的那个 $Q$ 决定音量。$Q_{\text{铺开}}$ 把 0.50 的概率放在中间，但 $P$ 在这里仅有 0.01，因此把大量概率压进 $P$ 的低谷会被重罚；最终“只押 A”的 KL 是 0.63，小于“铺开”的 1.61，所以 reverse KL 选择只押 A，表现出 **mode-seeking（寻找模式）**的倾向。只押 B 也可能得到类似结果，并不是 reverse KL 天生偏爱 A。
...
**credit assignment**要回答的是：最终得了 1 分或 0 分，前面哪些动作应该得到功劳（对最终结果是有益的），哪些动作应该被惩罚（对结果是有害的），因此我认为翻译成**“奖惩分配”**更加恰当。
