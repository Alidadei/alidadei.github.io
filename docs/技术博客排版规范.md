...
- **引用方式**:Astro 构建时 `public/` 的内容直接复制到站点根,引用路径从站点根开始,**去掉 `public/` 前缀**。
  - Markdown: `![描述](/images/posts/xxx.png)`
  - HTML(需缩放): `<img src="/images/posts/xxx.png" alt="描述" style="zoom:50%;" />`
- **不要用** `./../../../../public/images/...` 这种带 `public/` 的相对路径——构建后 `public/` 在 dist 里不存在,图片会 404。
- 例:文件物理位置 `public/images/posts/id-card.png` → 引用为 `/images/posts/id-card.png`。
