...
| categories | 否(默认回退 note) | string[] | 分类路径,**值必须是 categories.json 树里存在的 slug**,见下 |
...
1. frontmatter 是否在文件最开头,格式是否标准(title/date 必填;**categories 单行内联数组、tags 多行块**)
2. categories 的每个 slug 是否都在 `categories.json` 树里存在(不存在就是悬空)
3. 公式下标是否用了 `_{}`、希腊字母是否用了 LaTeX 命令(检查裸 `_xxx` 和裸 μ/σ/π)
4. 是否有残缺斜体标记(搜索 `*`)
5. 是否有孤立冒号行
6. **正文标题是否从 `##` 开始(不是 `#`)、层级是否逐级递减**
7. 分割线是否统一为 `---`
8. 列表空行是否清理
9. 错别字是否纠正
10. 发布前 `npm run build` 是否全绿
