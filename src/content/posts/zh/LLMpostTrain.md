...
本文将对以上几个常见后训练方法进行简介，同时对照 TRL 库的实际代码进行解读，方便随时复习。

## SFT

SFT是有监督微调的缩写，也就是给模型一个ground truth（GT）进行训练。对于LLM来说，GT一般是一段标准回复语（比如：客服针对某个问题的回答或者是某个工具调用的标准模板），用于让模型学会用特定的风格或模板来回答问题。
...
pretraining 阶段模型会对所有 token 逐个计算交叉熵损失（每个位置都学），但SFT阶段仅会对特定的token（GT内容）计算交叉熵损失。
...
其中，t代表一个token序列上的位置 ，p为LLM输出的该位置上词表 V 的概率分布 ，y是目标token（也就是GT内容）的 one-hot 向量（目标 token位置的值为 1，其余位置为 0），代表着GT内容的概率分布。因此，最终值为当前LLM在t位置上输出目标token的信息熵。

对于自回归语言模型，假设输入序列有 L 个 token，整个序列的loss 是所有位置的平均：
$$
\mathcal{L}_{\text{CE}} = \frac{1}{L} \sum_{t=1}^{L} \text{CE}_t = -\frac{1}{L} \sum_{t=1}^{L} \log(p_{\text{target\_token}})
$$

**如何理解交叉熵损失？** 交叉熵损失衡量的是两个分布之间信息量的重叠程度，交叉熵损失则计算的是模型输出的预测概率分布（softmax 后的 logits）与 **学习内容 token 的 one-hot 分布**之间的交叉熵。等价于让模型在每个位置给”正确的下一个 token”分配尽可能高的概率。

 SFT 阶段仅在 特定 回答语料部分计算这个 loss（prompt 部分会用 `labels[:prompt_len] = -100` 屏蔽，`ignore_index=-100` 跳过），为的就是让模型只学“怎么回答”，不去动它已有的通用知识——即在保留预训练能力的前提下，补充指令跟随的能力。
