---
title: '✒️万字长文：LLM 后训练基础知识&常见“黑话”整理'
date: 2026-07-18
tags:
  - 'LLM'
  - '后训练'
...
我在学习和实践LLM后训练的过程中，发现这其中存在许多“黑话”，它们其实是诸多LLM相关的基础概念，但是很容易遇到各种各样五花八门的说法，有时候不同叫法指的是同一个东西有时候指的却是不同的东西，一开始入门的时候很容易让人觉得晕头转向，因而我便产生了将它们全部梳理一遍的冲动。本文将根据论文和主流框架里相对稳定的共识来解释这些概念，想快速了解的读者可以不必深究公式，只需理解每个公式究竟在“比较谁/优化谁/约束谁”。

## 什么是 LLM 后训练

大语言模型首先会在海量文本上做**预训练**来吸收必要的知识，其本质是让模型通过 **自监督地预测下一个 token** 来学习文本中的各种规律，并将其压缩进模型参数里——所谓“自监督”，就是正确答案直接来自原文本的下一个 token，不需要人工逐条标注；之后，模型还需要学会贴近人的偏好、遵守某些特定的输出格式或者在特定任务上提高正确率，针对这些能力的训练可以统称为 **后训练（post-training）**。
...
OpenAI 的 [InstructGPT 论文](https://arxiv.org/abs/2203.02155) 采用的便是“SFT—偏好排序—奖励模型—强化学习”这条经典路线，不过“继续预训练算不算后训练”目前尚无统一口径：有人按时间顺序把基础预训练后的所有训练都算进去，有人把 continued pretraining 单独列为语料适配。个人认为“继续预训练”和 SFT、DPO 这类训练在数据形式和优化目的上都有明显区别，严格来说不应该看做后训练。

## 基础知识

📚下面将梳理一下我认为学习后训练需要掌握的LLM基础知识，已经有相关基础的读者可以跳过这一章节，或在阅读后面章节产生疑惑之时跳回来查阅。
...
**词表（vocabulary）** 就是 token 与 token ID 的对照表，此外，LLM 还有一个 embedding 矩阵，负责把每个 ID 变成向量。模型输出时，最后一层通常也要给词表中的每个 token 打分，因此 tokenizer、词表、输入 embedding 和输出头必须彼此对得上号。
...
| log-prob    | `log(0.42) ≈ -0.87` | 概率对数，用加减代替乘除，计算更加稳定   |
...
我们经常说的LLM“温度” 本质是作用在 softmax上的一个参数$T$：
...
所以，如果回答 $y$ 确实是从模型分布中采样出来的，$-\log p_\theta(y\mid x)$ 可以作为估计完整回答分布 entropy 的一个蒙特卡洛样本；但是对于这一条已经输出的回答，这个数本身叫 surprisal，直接把它叫做“这条回答的 entropy”并不准确。
...
工程日志里的 `entropy` 代表啥要看具体实现，当前 [TRL GRPOTrainer](https://huggingface.co/docs/trl/main/en/grpo_trainer) 记录的是“生成回答中各 token 预测分布的平均 entropy”：即沿着整条回答token序列，在每个生成位置计算一次整个词表的分布entropy，再对有效 token 求平均。以一条token数量为 $T$ 的回答为例：
...
中文翻译一般叫“KL 散度”，它衡量的是两个概率分布的差异有多大，**在这里，“divergence”主要指的是信息上的背离/偏离程度，它和物理学中向量场的散度完全不是一个意思**（所以我认为这个中文翻译并不好，不如叫“偏离度”呢）。KL divergence 的数学公式为：
...
1. 对每一种可能结果 $x$，先算概率比 $P(x)/Q(x)$。如果 $P$ 和 $Q$ 给它的概率相同，比值就是 1，取对数后就是 0，说明这一项没有分歧；
2. 再乘以 $P(x)$。这相当于让目标分布 $P$ 决定每种结果的权重：$P$ 认为经常会出现的结果，对最终 KL 的影响更大；
3. 最后把所有结果的贡献加起来，得到 $Q$ 整体偏离 $P$ 的程度。如果某个结果在 $P$ 中可能发生，即 $P(x)>0$，但 $Q$ 却给出 $Q(x)=0$，那就等于 $Q$ 把一个可能发生的结果当成绝不可能，KL 会变成无穷大。

为什么这能表示“信息上的背离”？因为在信息论里，一个结果发生时携带的信息量可以写成 $-\log p(x)$：原本越不可能发生的结果，一旦真的发生，就越让人意外，信息量也越大。于是，对同一个结果 $x$：

- 如果按照真实分布 $P$ 来描述它，信息量是 $-\log P(x)$；
- 如果错用分布 $Q$ 来描述它，信息量是 $-\log Q(x)$；
- 两者相减，正好就是 $\log\frac{P(x)}{Q(x)}$。

再按照真实分布 $P$ 对这个差值取平均，就得到：
...
**所以 KL divergence 也可以理解为：真实结果来自 $P$，却用分布 $Q$ 去理解或预测这些结果时，平均要多付多少“信息量”。** 使用自然对数时单位叫 **nat**，使用以 2 为底的对数时单位叫 **bit**。

例如 $P$ 认为 A、B 出现的概率分别是 0.9 和 0.1，$Q$ 却都猜成 0.5，那么：
...
这里 B 对应的第二项其实是负数，因为 $Q$ 给 B 的概率比 $P$ 更高；这不矛盾，KL 保证的是所有项加起来不小于 0，并不是每一项都不小于 0。最终的 0.368 也不是说“两个分布相差 36.8%”，而是：如果结果真的按照 $P$ 产生，使用 $Q$ 而不是 $P$ 来描述结果，平均每次会多付约 0.368 nat 的信息量。这个“平均多付的信息量”越接近 0，$Q$ 就越贴近 $P$。[Stanford EE376A 信息论讲义](https://web.stanford.edu/class/ee376a/files/lecture_notes.pdf)也将 KL 写成 $\mathbb E_P[\log(P/Q)]$，并称为 relative entropy（相对熵）。

KL divergence 有三个重要性质：
...
只保留概率最高的 $k$ 个 token，把其他 token 的概率设为 0，再重新归一化并做多项式采样。它的目的是：**砍掉低概率长尾，避免一次随机抽样把模型带到非常离谱的 token 上**。

- $k$ 越小，候选越少，输出通常越集中、稳定，但也更容易模板化或重复；
- $k$ 越大，越接近在完整词表上直接采样，输出更多样，但抽到不合适 token 的机会也会增加；
- 当 $k=1$ 时只剩概率最大的 token，结果就退化成贪婪解码。

它的局限也很明显：候选数量永远固定。模型很确定时，第 1 名可能已经占 95%，保留 50 个候选显得过多；模型很不确定时，前 50 个 token 又可能只覆盖很少的总概率。
...
temperature、top-k 和 top-p 可以一起使用：在常见的采样管线中，temperature 会先改变概率差距，进而影响哪些 token 能进入 top-k / top-p 集合。
...
- **sample / example**：一个训练样本。它可以指“一条 prompt”“一条 prompt-completion”或“一个 token”。
...
- **rollout** 更强调“让某个策略（LLM）实际跑一次并采样”的过程，也常指这次生成本身；对应到大模型来说，就是“回答问题”的过程。
...
也就是说，LLM 每输出一个 token，就执行了一次 action；这个 token 被接到原有上下文后，又形成下一步的 state。当模型输出 stop token 后，这次生成结束，也就得到了一条完整 trajectory。若生成因最大长度或外部控制器而提前停止，则以实际停止的位置作为轨迹终点。Agent 训练任务的轨迹还可能包含“调用搜索工具—收到结果—继续思考—提交答案”等多轮动作。

### Batch、minibatch、microbatch 与 train_batch

先前[GRPO 训练中的量词简介 | Harry Yu](https://alidadei.github.io/zh/blog/grpo-training-quantifiers/)这篇博客用形象的例子解释了在简单GRPO训练中batch、step和epochs之间的关系。

不过广义来看，`train_batch` 并不是一个跨框架统一的标准术语。看到它时，至少要追问两个问题：
...
- **rollout / generation batch**：一次LLM集中生成并打分的数据；
- **epoch**：把指定数据完整重复训练一遍。PPO 中也常指同一 rollout batch 被切成 minibatch 重用若干轮。

在纯数据并行、没有额外样本复制的简单设置下，若 $B_{\mathrm{device}}$ 明确代表“每张设备每个 micro step 的样本数”，那么：
...
例如每张卡每次处理 4 条 completion，2 个数据并行副本，累积 8 次梯度，那么一次 optimizer update 汇总 $4\times2\times8=64$ 条 completion。这里其实叠加了两种扩大 batch 的办法：

- **2 个数据并行副本**：两张卡各放一份参数相同的 LLM，同时处理各自拿到的数据，最后合并梯度。它们通常拿到的是不同训练样本，GRPO 可能把同一 prompt 的多条不同 completion 分到不同设备上，但这是 sampler 怎样分组的结果，不是数据并行强制要求的。
  - 这里的 **sampler** 不是 LLM 生成 token 时的概率采样，而是数据加载阶段的“取号员”：它决定从数据集中取哪些样本、按什么顺序取，以及怎样组成 batch。普通 sampler 通常逐个给出样本索引，batch sampler 则一次给出一组索引。在 GRPO 里，sampler 还要照顾“同一条 prompt 生成 $G$ 条 completion”的分组要求。
- **累积 8 次梯度**：每张卡连续处理 8 个 microbatch，每次都做前向和反向、把新梯度累加到已有梯度上，但前 7 次都不更新模型参数；第 8 次结束后才让优化器更新一次。这样能在显存放不下 64 条 completion 的情况下，让一次更新综合 64 条 completion 的信号。[Hugging Face Accelerate 文档](https://huggingface.co/docs/accelerate/usage_guides/gradient_accumulation)给出的定义也是“跨多个 batch 累积梯度，再执行一次 optimizer step”。

```text
一个 micro step：GPU 0 处理 4 条 completion ─┐
                GPU 1 处理 4 条 completion ─┴→ 本轮共 8 条 completion 的梯度
                                                   ↓ 不更新参数，继续累加
重复 8 个 micro step ─────────────────────────────→ 汇总 64 条 completion
                                                   ↓ optimizer step
                                              更新一次 LLM 参数
```

这里一条 **completion** 就是前文说的一条完整回答，不是一道问题。64 条 completion 表示 64 条回答；如果 GRPO 对每条 prompt 正好生成 $G=8$ 条回答，而且这 64 条回答由 8 个完整分组组成，那么它们就对应 8 条 prompt（8个问题）：

$$
B_{\mathrm{prompt}}=\frac{B_{\mathrm{completion}}}{G}=8
$$

但不同框架可能在 sampler 中先把 prompt 按 $G$ 重复，也可能先取一批互不相同的 prompt，再在生成阶段各扩成 $G$ 条 completion。因此，只看到 `per_device_train_batch_size`，不能断定它代表多少条独立 prompt。TRL 的 [GRPO 配置说明](https://huggingface.co/docs/trl/grpo_trainer) 会额外规定 effective batch 与 `num_generations` 的整除关系，并单独定义 generation batch。

#### Step 也有多种含义

- **generation step**：自回归生成一个 token；
- **micro step**：处理一个 microbatch，完成一次前向和反向并得到一份梯度；在梯度累积期间，这一步不会立刻更新模型参数；
- **optimizer step / update step**：真正调用优化器更新一次参数；
- **environment step**：智能体在环境里执行一次动作。

如果只写 `training steps`，在常见 LLM 训练框架里通常沿用 **update step / global step** 的口径，也就是若干个 micro step 完成梯度累积后，优化器更新一次参数；例如 [Hugging Face Trainer 文档](https://huggingface.co/docs/transformers/main_classes/trainer)把 `eval_steps` 明确定义为两次评估之间的 update step 数量。但在在线强化学习论文里，外层 step 可能更大：比如 [DeepSeekMath 的 GRPO 算法 1](https://arxiv.org/abs/2402.03300) 中，一个外层 `step` 包含采样 batch、为每条问题生成 $G$ 个回答、计算奖励，以及随后进行 $\mu$ 轮 模型更新。因此，光看论文写“训练了 1000 steps”仍然不够，必须结合算法伪代码和训练配置，确认它指的是 optimizer update、外层 RL 循环，还是作者另外定义的 step。
...
- **当前策略（current policy）**：就是此刻正在被优化器更新的那个模型，记作 $\pi_\theta$。current policy具有很强的时效性：只要模型的参数一更新，current policy就变了，它在相同上下文里给出的下一 token 概率也会变化。
...
$\mu$ 是真正生成这份数据的 behavior policy（LLM），$\pi_{\mathrm{target}}$ 是我们想估计或训练的目标策略（LLM）。这里**采样得到的不是概率分布，而是一个具体 token**：在状态 $s_t$ 下，$\mu$ 先给整个词表输出一份概率分布，再从中抽出 token $a_t$。rollout 数据会记录这条 token 序列，并且通常还会保存每个已抽中 token 在旧策略下的 logprob，供训练时计算 ratio。

例如旧策略在第 $t$ 步给最终抽中的 token“因此”分配了 0.10 的概率，current policy 训练后给同一个 token 分配了 0.15，那么 ratio 就是 $0.15/0.10=1.5$。它是在重新衡量**这一个已经采到的 action**，不是拿两份完整词表分布直接相除。
...
这就是大家口中的 **old-policy ratio**。下标 $t$ 表示生成过程中的第 $t$ 步，也就是第 $t$ 个 token 所在的位置，并不代表一份概率分布；$r_t(\theta)$ 表示这一步实际抽中的 token 的新旧概率比。它大于 1，说明 current policy 现在比 old policy 更容易选中这个 action；小于 1，则说明现在更不容易选中。在 LLM 里，$a_t$ 通常就是第 $t$ 个输出 token，$s_t$ 就是 prompt 加上前面已经输出的 token。[OpenAI Spinning Up 的 PPO 公式](https://spinningup.openai.com/en/latest/algorithms/ppo.html)比较的也是同一个状态—动作对在新旧策略下的概率。
...
💡它和 KL divergence 不是一回事：importance ratio 针对**这一个实际采到的 action**做概率校正，而 KL 是对一整个概率分布偏离训练起点程度的衡量。同时PPO 会裁剪 $\pi_\theta/\pi_{\mathrm{old}}$，让过大的 ratio 不再继续放大这一项训练目标，从而抑制单次更新过猛。
...
`online` 说的是“**数据是否边练边采**”，`on-policy` 说的是“**训练数据的概率分布是否来自current policy（即当前送去计算奖励的这个回答是否是由正在训练的这个LLM生成的）**”。 PPO 为了提高数据利用效率，常先用旧策略快照（参数更新前的LLM）生成一批回答，再用这批回答切成 minibatch 训练几个 epoch（计算奖励并更新模型）；在这几个 epoch 中，当前策略已经发生变化（模型被更新了），所以工程上常说它是近似 on-policy。[OpenAI Spinning Up 的算法分类](https://spinningup.openai.com/en/latest/spinningup/rl_intro2.html) 将 PPO 列为 on-policy 方法，并强调这类方法不能随意使用旧策略数据。
...
这里的 $x$ 是同一个 prompt，$y_w$ 是人类更喜欢的回答（chosen / winner），$y_l$ 是不太喜欢的回答（rejected / loser）。奖励模型先分别给两个回答打分，再用 chosen 的分数减去 rejected 的分数；这个公式只在意两个分数的**差**，所以绝对值不具有比较意义。其中$\sigma$ 是 sigmoid 函数，负责把这个分数差压到 0～1 之间，变成“chosen 胜过 rejected 的概率”。

![Sigmoid 把奖励分数差映射成偏好概率](../../../../public/images/posts/reward-model-sigmoid.svg)
...
在 LLM 文章里一般更常看到 reward、很少看到 return，是因为很多单轮 LLM 任务只在完整回答结束后给一个 reward；如果中间没有其他奖励，而且 $\gamma=1$，那么从任意 token 位置往后累计，得到的 return 都等于最后那个 reward。两个数字刚好一样，文章就常直接说“用最终 reward 训练”，不再单独把 return 拎出来讲。
...
value 预测的是未来回报，可作为基线来降低梯度方差。PPO 常额外训练一个 **critic / value model** 估计它，而GRPO则直接用同一 问题下的一组回答的奖励平均值做估计，不用专门训练一个critic模型。
...
翻译成人话就是：**这一步实际拿到的 reward，加上下一个状态预计还能拿到的回报（由专门的critic model估计），和当前状态原本预计能拿到的回报，差了多少。** GAE 再把当前位置和后面位置的这些误差按距离逐渐打折后加起来：
...
$\lambda$ 决定“往后看多远”：越小越依赖 value model 的短期预测，估计通常更稳但偏差可能更大；越接近 1，越依赖后面真实发生的 reward，偏差更小但波动可能更大。所以 GAE 是一种把 reward 和 value 整理成最终用于更新模型的 advantage 的办法。详细推导可见 [GAE 原论文](https://arxiv.org/abs/1506.02438)。

而GRPO 的关键做法则是：不训练 critic 模型，直接把同一 prompt（问题）下的一组回答分数的均值当做相对基线（value）。
...
**credit assignment**要回答的是：最终得了 1 分或 0 分，前面哪些动作应该得到功劳（对最终结果是有益的），哪些动作应该被惩罚（对结果是有害的），因此我认为翻译成**“奖惩分配”**比较恰当。

如果一篇很长的数学推理只有最终答案奖励，最朴素的做法会把同一个优势信号分给整段回答的所有 token，但是这样没法知道具体哪些token是有用的，因此便需要考虑寻找出对最终求解有用/无用的token。[LLM RL 中的 Credit Assignment 问题 | Harry Yu](https://alidadei.github.io/zh/blog/credit_assignment_in_llm_rl/)这篇博客中有一些具体的例子。
...
这里的 $\theta$ 就是 current policy，也就是那份正在训练的LLM的参数。这个公式更新的是 policy / actor，不会顺手更新冻结的 reference model 或 reward model；PPO 的 value model 另有自己的 value loss。
...
这里用到了`log probability` ，是因为序列概率由许多条件概率相乘；取对数后可以把它们变成相加，求导也更方便。策略梯度的基础推导可参考 [Spinning Up 的说明](https://spinningup.openai.com/en/latest/spinningup/rl_intro3.html)。

### 一张易混概念对照表

| 容易混淆的词 | 最短区分 |
| --- | --- |
| token / word | token 是 tokenizer 定义的单位，不等于自然语言里的词 |
| autoregressive / greedy | 前者是按前缀逐步预测；后者是每步取最大概率的一种解码 |
| temperature / entropy bonus | 前者影响推理采样；后者进入训练目标（loss）并影响模型权重 |
| reward / return | reward 是某一步或终局得分；return 是从轨迹第 $t$ 步（LLM 生成第 $t$ 个 token 的位置）起累计到回答结束的奖励 |
| reward / advantage | reward 是原始分数；advantage 是相对一个估计的reward基线好多少 |
...
| online / on-policy | 前者说是否边训练边采数据；后者说计算奖励的数据是否来自当前策略（LLM） |
...
2. **数据从哪里来？** 固定数据集，还是依靠当前/旧策略（LLM）新生成的 rollout？
3. **一条训练样本是什么？** prompt、completion、pair、trajectory 还是 token？
4. **用于评价一次生成的奖励从哪里来？** 人类偏好模型、AI judge、程序 verifier，还是几者组合？
5. **用于更新模型的优势怎样算？** critic、GAE、组内均值，还是整条回答同一个分数？
6. **有哪些约束？** current-old 的 ratio，还是 current-reference 的 KL？
7. **一次更新实际用了多少数据？** 写清设备数、microbatch、梯度累积、group size 和单位。
8. **同一 rollout 重用了几次？** 数据越旧，当前策略与行为策略（behavior/old policy) 的偏差通常越大。
9. **怎样评估？** 不能只看训练 reward；还要拿一批训练时没有用过的新题测试模型——这批新题就叫“留出任务”，此外还要检查格式、长度、旧能力、安全性并做人工抽检。
